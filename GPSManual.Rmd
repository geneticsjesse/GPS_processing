...
# Generate some sort of boundaries if you see obviously erroneous points in plot above
lowEast <- 600000; highEast <- 640000
lowNorth <- 6000000; highNorth <- 6300000
...
#Animalid is unique ID identifying individual MG
...
# Output pdf of locs xy by herd BEFORE movement rate cutoff and flag the locs----
...
mogo_winter2018 <- gps_simp_clean2_dt %>%
  filter(LMT_Date >= as.Date("2017-12-01") & LMT_Date <= as.Date("2018-04-30"))

mogo_winter2019 <- gps_simp_clean2_dt %>%
  filter(LMT_Date >= as.Date("2018-12-01") & LMT_Date <= as.Date("2019-04-30"))

mogo_winter2020<- gps_simp_clean2_dt %>%
  filter (LMT_Date >=as.Date("2019-12-01") & LMT_Date <= as.Date("2020-04-30"))

mogo_winter <- rbind(mogo_winter2018, mogo_winter2019, mogo_winter2020)
#write.csv(mogo_winter, "output/winterGPS.csv")


#### Setting up seasonal data sets - SUMMER 
mogo_summer2018 <- gps_simp_clean2_dt %>%
  filter(LMT_Date >= as.Date("2018-05-01") & LMT_Date <= as.Date("2018-10-31"))

mogo_summer2019 <- gps_simp_clean2_dt %>%
  filter(LMT_Date >= as.Date("2019-05-01") & LMT_Date <= as.Date("2019-10-31"))

mogo_summer2020 <- gps_simp_clean2_dt%>%
  filter (LMT_Date >=as.Date("2020-05-01") & LMT_Date <= as.Date("2020-10-31"))

mogo_summer <- rbind(mogo_summer2018, mogo_summer2019, mogo_summer2020)
#write.csv(mogo_winter, "output/summerGPS.csv")
```
